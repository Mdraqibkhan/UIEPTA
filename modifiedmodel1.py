...
def Phase_swap(x,y):
  ...
  out=torch.fft.ifftn(abs(fftn2)*torch.exp(1j*(fftn1.angle()))) #pHASE SWAPPING 
  return out.real
...
class Model(nn.Module):
    def __init__(self, num_blocks=[2, 3, 3, 4], num_heads=[1, 2, 4, 8], channels=[16, 32, 64, 128], num_refinement=4,
        ...
        self.sig=nn.Sigmoid()
        self.to_1x1 = nn.ModuleList([attention_to_1x1(num_ch) for num_ch in ch])
       
        self.embed_conv_rgb = nn.Conv2d(3, channels[0], kernel_size=3, padding=1, bias=False)
        self.embed_conv_gray = nn.Conv2d(1, channels[0], kernel_size=3, padding=1, bias=False)
    ...
    def forward(self,RGB_input,Gray_input):
        ...
        out_enc_rgb1 = self.encoders[0](self.sig(self.to_1x1[0](fo_gray)*fo_rgb))
        out_enc_gray1 = self.encoders[0](fo_gray)
        out_enc_rgb2 = self.encoders[1](self.downs[0](self.sig(self.to_1x1[1](out_enc_gray1)*out_enc_rgb1)))
        out_enc_gray2 = self.encoders[1](self.downs[0](out_enc_gray1))
        out_enc_rgb3 = self.encoders[2](self.downs[1](self.sig(self.to_1x1[2](out_enc_gray2)*out_enc_rgb2)))
        out_enc_gray3 = self.encoders[2](self.downs[1](out_enc_gray2))
        out_enc_rgb4 = self.encoders[3](self.downs[2](self.sig(self.to_1x1[3](out_enc_gray3)*out_enc_rgb3)))
  
        ###-------Dencoder------####
      
        OUT1=Phase_swap(out_enc_rgb3,self.ups[0](out_enc_rgb4))
        out_dec3 = self.decoders[0](self.reduces[0](torch.cat([self.ups[0](out_enc_rgb4), OUT1], dim=1)))
        OUT2 =Phase_swap(out_enc_rgb2,self.ups[1](out_dec3))
        out_dec2 = self.decoders[1](self.reduces[1](torch.cat([self.ups[1](out_dec3), OUT2], dim=1)))
